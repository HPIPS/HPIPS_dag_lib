...
void dag_hash(void *data, size_t size, dag_hash_t hash)
{
	SHA256REF_CTX ctx;
	...
	sha256_init(&ctx);
	sha256_update(&ctx, (uint8_t*)hash, sizeof(dag_hash_t));
	sha256_final(&ctx, (uint8_t*)hash);
}

//获取哈希地址大小
unsigned dag_hash_ctx_size(void)
{
	return sizeof(SHA256REF_CTX);
}

//哈希值初始化
void dag_hash_init(void *ctxv)
{
...
//哈希值更新
void dag_hash_update(void *ctxv, void *data, size_t size)
{
	SHA256REF_CTX *ctx = (SHA256REF_CTX*)ctxv;
...
//哈希的最终结果
void dag_hash_final(void *ctxv, void *data, size_t size, dag_hash_t hash)
{
	SHA256REF_CTX ctx;
...
//Dag 获取Hash的状态
void dag_hash_get_state(void *ctxv, dag_hash_t state)
{
	SHA256REF_CTX *ctx = (SHA256REF_CTX*)ctxv;

	memcpy(state, ctx->state, sizeof(dag_hash_t));
}

//设置哈希状态
void dag_hash_set_state(void *ctxv, dag_hash_t state, size_t size)
{
	SHA256REF_CTX *ctx = (SHA256REF_CTX*)ctxv;

	memcpy(ctx->state, state, sizeof(dag_hash_t));
	ctx->datalen = 0;
	ctx->bitlen = size << 3;
	ctx->bitlenH = 0;
